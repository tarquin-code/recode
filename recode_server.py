...
ENCODE_TAG_RE = re.compile(r"_h26[45]_[A-Za-z0-9-]+(?:_[A-Za-z0-9-]+)?$")  # matches our encoded output stems (2 or 3 parts)


def build_encode_tag(video_codec: str, info: dict, dv_mode: str = "skip") -> str:
    """Build the filename tag for encoded output, e.g. '_h265_DV-P8_Streamer'."""
    ...
    return f"_{codec_tag}_{hdr_tag}_{suffix}"
...
def build_ffmpeg_cmd(info: dict, settings: dict) -> tuple[list[str], str]:
    ...
    encode_tag = build_encode_tag(video_codec, info, dv_mode)
...
class EncodeQueue:
    ...
    def is_duplicate(self, file_info: dict) -> bool:
        """Check if a file is already queued or actively encoding."""
    ...
    def add(self, file_info: dict, settings: dict) -> EncodeJob:
        if self.is_duplicate(file_info):
            return None
...
def cleanup_tmp_dir():
    ...
    for jid, job in encode_queue.active_jobs.items():
        ...
        encode_tag = build_encode_tag(settings.get("video_codec", "hevc"), info, settings.get("dv_mode", "skip"))
...
async def queue_add(req: QueueAddRequest):
    base_settings = {
        ...
        "speed": req.speed, "use_cpu": req.use_cpu, "gpu_id": req.gpu_id, "dv_mode": req.dv_mode,
    ...
    await manager.broadcast({"type": "state_update", "data": encode_queue.get_state()})
    result = {"added": len(added), "job_ids": added}
    if skipped_dupes:
